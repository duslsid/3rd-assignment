...
/* 장바구니에 담은 이후에도 옵션을 바꿀 수 있도록 드롭다운으로 표시한다 */
function optionHtml(item, index) {
  const sizes = cartSizes(item.sizeCase);
...
(async function () {
  ...
  function render() {
    ...
    if (!cart.length) {
      list.innerHTML = '<p class="cart-empty font-pname">장바구니가 비어 있습니다.</p>';
  ...
  /* 수량이 바뀌거나 옵션이 바뀌면 같은 상품끼리 다시 합친 뒤 저장한다 */
  function save(cart) {
    const merged = [];
    cart.forEach(function (item) {
      if (item.qty > 0) mergeCartItem(merged, item);
    });
  ...
  list.addEventListener('change', function (e) {
    ...
    const cart = readCart();
    const item = cart[Number(select.dataset.index)];
    if (!item) return;
